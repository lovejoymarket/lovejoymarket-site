(() => {
  const COUNTER_NAMESPACE = 'lovejoymarket-co';
  const COUNTER_NAME = 'homepage-visits';
  ...
  const formatCount = (value) => String(Math.max(0, Math.trunc(Number(value) || 0))).padStart(6, '0');
  ...
  const render = (value) => {
    ...
    holder.setAttribute('aria-label', `visitor number ${Number(value)}`);
  };
  ...
  const loadCounterLibrary = () => new Promise((resolve, reject) => {
    if (window.Counter) {
      resolve(window.Counter);
      return;
    }
    ...
    script.async = true;
    script.setAttribute('data-counterapi-library', '');
    script.onload = () => window.Counter
      ? resolve(window.Counter)
      : reject(new Error('CounterAPI library loaded without Counter global'));
    script.onerror = () => reject(new Error('CounterAPI library failed to load'));
    document.head.appendChild(script);
  });

  const getClient = async () => {
    if (counterClient) return counterClient;
    const Counter = await loadCounterLibrary();
    counterClient = new Counter({
      version: 'v1',
  ...
  const requestCount = async (increment = false) => {
    ...
    const result = increment
      ? await client.up(COUNTER_NAME)
      : await client.get(COUNTER_NAME);

    const value = result?.value ?? result?.count ?? result?.data?.value ?? result?.data?.count;
    if (!Number.isFinite(Number(value))) {
      throw new Error('CounterAPI response did not include a numeric value');
    }
    return Number(value);
  };
